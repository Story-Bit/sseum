  ...
  const handleInitialAnalysis = async () => {
    ...
    try {
      const response = await fetch('/api/gemini', {
        ...
        // [오류 수정] 신탁소가 요구하는 모든 데이터(mainKeyword, blogType)를 payload에 담아 전송한다.
        body: JSON.stringify({ 
          task: 'analyzeKeywords', 
          payload: { mainKeyword, blogType } 
        }),
      });
      if (!response.ok) throw new Error((await response.json()).error || "API 서버 오류");
      const data = await response.json();
      
      setKosResults(data.kosResults);
      if (data.kosResults && data.kosResults.length > 0) {
        handleKeywordSelect(data.kosResults[0].keyword); 
      }
          ...
          <>
            <section>
              ...
              <Card className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 p-6">
                ...
                <Button onClick={handleGeneratePillarPost} className="mt-4">이 전략으로 초고 생성</Button>
              </Card>
