...
const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro-latest", safetySettings: [/* ... */], generationConfig: { responseMimeType: "application/json" } });
...
const createPrompt = (task: string, payload: any): string => {
  ...
  switch (task) {
    ...
    case 'generateStrategyDetails':
      return `${systemPrompt}\n\n키워드 '${payload.selectedKeyword}'를 중심으로, 주제적 권위를 구축하기 위한 콘텐츠 전략을 아래 JSON 구조로 제안하시오. [삭제] pillarContent는 생성하지 않습니다.\n\n{"topicClusters": [{"mainTopic": "string", "subTopics": ["string"]}], "personas": [{"name": "string", "description": "string", "recommendedPosts": [{"title": "string", "tactic": "string"}]}]}`;

    // [삭제] 'generatePillarPost' 임무를 폐기함
    ...
    case 'analyzeCompetitor':
      return `${systemPrompt}\n\n[경쟁사 본문]\n${payload.competitorContent}\n\n[출력 형식]\n{"analysis": "string", "suggestedTitles": ["string"], "suggestedOutline": "string"}`;
